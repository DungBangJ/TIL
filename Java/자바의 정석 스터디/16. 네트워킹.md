    ...
    public static void main(String[] args) {
        ...
        while (true) {
            try {
                ...
                //Socket클래스에 정의된 getPort()와 getLocalPort()를 사용해서
                //TCP/IP통신에서 소켓이 사용하고 있는 포트를 알아낼 수 있다.
                System.out.println("getPort(): " + socket.getPort()); //상대편 소켓(원격 소켓)이 사용하고 있는 포트
                System.out.println("getLocalPort(): " + socket.getLocalPort()); //소켓 자신이 이용하는 포트
...
<cmd>
D:\study\Codes\YJ\src\YJ16>javac YJ16_08.java

D:\study\Codes\YJ\src\YJ16>java YJ16_08
[08:45:10]server ready.
[08:45:10]waiting for connection.
```
...
<cmd>
D:\study\Codes\YJ\src\YJ16>javac YJ16_09.java

D:\study\Codes\YJ\src\YJ16>java YJ16_09
[08:58:38]server ready.
[08:58:38]waiting for connection.
timeout for connection, ending server.
```

서버에 접속하는 클라이언트의 수가 많을 때 여러 쓰레드를 이용해서 클라이언트의 요청을 병렬적으로 처리하기
...
public class YJ16_10 implements Runnable {
    ServerSocket serverSocket;
    Thread[] threadArr;

    public static void main(String[] args) {
        YJ16_10 server = new YJ16_10(5);
        server.start();
    }//main

    public YJ16_10(int num) {
        try {
            serverSocket = new ServerSocket(7777);
            System.out.println(getTime() + "server ready");
    ...
    public void run() {
        while (true) {
            try {
                System.out.println(getTime() + "is waiting for request");

                Socket socket = serverSocket.accept();
                System.out.println(getTime() + socket.getInetAddress() + " requests for connection");
...
<cmd>
D:\study\Codes\YJ\src\YJ16>javac YJ16_10.java

D:\study\Codes\YJ\src\YJ16>java YJ16_10
[09:31:19mainserver ready
[09:31:19Thread-0is waiting for request
[09:31:19Thread-4is waiting for request
[09:31:19Thread-1is waiting for request
[09:31:19Thread-2is waiting for request
[09:31:19Thread-3is waiting for request

D:\study\Codes\YJ\src\YJ16>java YJ16_10
[09:31:32mainserver ready
[09:31:32Thread-2is waiting for request
[09:31:32Thread-1is waiting for request
[09:31:32Thread-3is waiting for request
[09:31:32Thread-4is waiting for request
[09:31:32Thread-0is waiting for request
```

- 여러 개의 쓰레드를 생성해서 클라이언트의 요청을 동시에 처리하도록 했다.
  - 클라이언트의 요청이 많다면 병렬적으로 처리하는게 좋다.
  - 병렬적으로 처리하지 않으면 서버가 접속을 순서대로 처리하기 때문에 늦게 접속을 요청한 클라이언트는 오래 기다리게 된다.

서버 프로그램과 클라이언트 프로그램의 화면에 입력한 데이터가 상대방의 화면에 출력되므로 1:1 채팅이 가능하다.
...
<cmd>
D:\study\Codes\YJ\src\YJ16>javac YJ16_11.java

D:\study\Codes\YJ\src\YJ16>java YJ16_11
server ready.
Hello
[/127.0.0.1:7777]안녕
[/127.0.0.1:7777]여기는 인텔리제이 콘솔이야
와 신기하다 여기는 cmd 콘솔이야
대박 이게 연결이 되네
...
<Console>
connected to server
[/127.0.0.1:57259]Hello
안녕
여기는 인텔리제이 콘솔이야
[/127.0.0.1:57259]와 신기하다 여기는 cmd 콘솔이야
[/127.0.0.1:57259]대박 이게 연결이 되네
```
...
public class YJ16_13 {
    ...
    //클라이언트가 추가될 때마다 생성되며 클라이언트의 입력을 서버에 접속된 모든 클라이언트에게 전송한다.
    class ServerReceiver extends Thread {
        ...
        //클라이언트가 새로 추가되었을 때 클라이언트의 이름을 key로 클라이언트의 출력스트림을 HashMap인 clients에 저장해서 다른 클라이언트가 입력한 데이터를 전송하는데 사용한다.
        public void run(){
            String name = "";
            ...
            try {
                ...
                System.out.println("Headcount: " + clients.size());
                ...
                sendToAll("#" + name + " exit");
                clients.remove(name);
                System.out.println("[" + socket.getInetAddress() + ":" + socket.getPort() + "]" + " connection ended.");
                System.out.println("Headcount: " + clients.size());
            }
        }//run
    }//ServerReceiver
...
- 서버에 접속한 클라이언트를 HahsMap에 저장해서 관리
  - 동기화처리
